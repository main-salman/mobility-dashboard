...
const SafetyAnalysisPanel = ({ selectedCity }: SafetyAnalysisPanelProps) => {
  const [safetyFilter, setSafetyFilter] = useState<'all' | 'pedestrian' | 'bicycle' | 'vehicle'>('all');
  ...
  const safetyData = {
    incidentHotspots: [
      { id: 1, location: `${selectedCity.name} Main St & 5th Ave`, ...getRandomMetrics(), riskLevel: 'high' },
      { id: 2, location: `${selectedCity.name} University Blvd & Park St`, ...getRandomMetrics(), riskLevel: 'high' },
      { id: 3, location: `${selectedCity.name} Downtown Shopping District`, ...getRandomMetrics(), riskLevel: 'high' },
      { id: 4, location: `${selectedCity.name} West End & River Road`, ...getRandomMetrics(), riskLevel: 'medium' },
      { id: 5, location: `${selectedCity.name} Central Park Entrance`, ...getRandomMetrics(), riskLevel: 'medium' }
    ],
    accessibilityIssues: [
      { location: `${selectedCity.name} Main St Sidewalks`, issue: 'Uneven pavement', affectedGroups: 'Wheelchair users, Elderly', priority: 'high' },
      { location: `${selectedCity.name} Downtown Crosswalks`, issue: 'Insufficient crossing time', affectedGroups: 'Elderly, Visually impaired', priority: 'high' },
      { location: `${selectedCity.name} Transit Stations`, issue: 'Limited ramp access', affectedGroups: 'Wheelchair users', priority: 'medium' },
      { location: `${selectedCity.name} University District`, issue: 'Missing audible signals', affectedGroups: 'Visually impaired', priority: 'medium' }
    ],
    safetyImprovements: [
      { type: 'Infrastructure', action: `Install pedestrian islands at 3 major ${selectedCity.name} intersections`, impact: 'Reduces crossing distance and exposure time' },
      { type: 'Signalization', action: `Extend pedestrian crossing times at 5 ${selectedCity.name} downtown intersections`, impact: 'Accommodates slower-moving pedestrians' },
      { type: 'Design', action: `Add protected bike lanes on ${selectedCity.name} Main St corridor`, impact: 'Separates bicycles from vehicle traffic' },
      { type: 'Enforcement', action: `Increase speed limit enforcement in ${selectedCity.name} school zones`, impact: 'Reduces vehicle speeds in high-pedestrian areas' }
    ],
    nighttimeSafetyMetrics: {
      poorlyLitAreas: [`${selectedCity.name} West End Commercial District`, `${selectedCity.name} University Path`, `${selectedCity.name} Riverside Park`],
  ...
  return (
    <div className="bg-white shadow-lg rounded-lg p-4 md:p-6 mb-6">
                ...
                <tr key={hotspot.id} className="hover:bg-gray-50">
                  ...
                  <td className="py-2 px-4 border-b text-center">
                    <span className={`inline-block w-8 h-8 rounded-full ${
                      hotspot.pedestrianIncidents > 10 ? 'bg-red-100 text-red-800' : 
                      hotspot.pedestrianIncidents > 5 ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'
                  ...
                  <td className="py-2 px-4 border-b text-center">
                    <span className={`inline-block w-8 h-8 rounded-full ${
                      hotspot.bicycleIncidents > 10 ? 'bg-red-100 text-red-800' : 
                      hotspot.bicycleIncidents > 5 ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'
                    } flex items-center justify-center`}>
                  ...
                  <td className="py-2 px-4 border-b text-center">
                    <span className={`inline-block w-8 h-8 rounded-full ${
                      hotspot.vehicleIncidents > 10 ? 'bg-red-100 text-red-800' : 
                      hotspot.vehicleIncidents > 5 ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'
                    } flex items-center justify-center`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="mt-3">
              ...
              <div className="flex items-center mt-1">
                <div className="w-full bg-gray-200 rounded-full h-2 mr-2">
                  <div className="bg-indigo-600 h-2 rounded-full" style={{ width: `${safetyData.nighttimeSafetyMetrics.pedestrianActivityDrop}%` }}></div>
                </div>
                <span className="text-sm">{safetyData.nighttimeSafetyMetrics.pedestrianActivityDrop}%</span>
              </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {safetyData.nighttimeSafetyMetrics.recommendedImprovements.map((improvement, index) => (
                <li key={index} className="flex items-start">
      ...
      <div className="mt-6 p-3 bg-blue-50 text-blue-700 text-sm rounded-md">
        ...
        <p>This analysis identifies high-risk areas for pedestrians, cyclists, and vehicles. Use this data to prioritize safety improvements and accessibility enhancements. The nighttime analysis can help address safety concerns that may be reducing evening mobility in certain areas.</p>
      </div>
    </div>
  );
};
