...
const CitySearch = ({ onCitySelect }: CitySearchProps) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setPlacesError("Failed to initialize Places API. Please use the quick select options instead.");
    }
    ...
    <div className="w-full">
      {placesError ? (
        <div className="mb-3 p-2 bg-yellow-50 border border-yellow-200 rounded text-yellow-800 text-xs">
          <p><strong>Note:</strong> {placesError}</p>
          <p className="mt-1">To fix: Enable the Places API in your <a href="https://console.cloud.google.com/apis/library/places-backend.googleapis.com" target="_blank" rel="noopener noreferrer" className="underline">Google Cloud Console</a>.</p>
        </div>
      ) : null}
